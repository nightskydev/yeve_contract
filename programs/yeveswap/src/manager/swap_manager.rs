...
use crate::{
    errors::ErrorCode,
    manager::{
        tick_manager::next_tick_cross_update, yevepool_manager::next_yevepool_reward_infos,
    },
    math::*,
    state::*,
...
pub struct PostSwapUpdate {
    ...
    pub next_reward_infos: [YevepoolRewardInfo; NUM_REWARDS],
...
pub fn swap(
    ...
    if a_to_b && sqrt_price_limit > yevepool.sqrt_price
        || !a_to_b && sqrt_price_limit < yevepool.sqrt_price
    {
    ...
    let tick_spacing = yevepool.tick_spacing;
    let fee_rate = yevepool.fee_rate;
    let protocol_fee_rate = yevepool.protocol_fee_rate;
    ...
    let mut curr_sqrt_price = yevepool.sqrt_price;
    let mut curr_tick_index = yevepool.tick_current_index;
    let mut curr_liquidity = yevepool.liquidity;
        ...
        if swap_computation.next_price == next_tick_sqrt_price {
            ...
            if next_tick_initialized {
                let (fee_growth_global_a, fee_growth_global_b) = if a_to_b {
                    (curr_fee_growth_global_input, yevepool.fee_growth_global_b)
                } else {
                    (yevepool.fee_growth_global_a, curr_fee_growth_global_input)
                };
    ...
    let fee_growth = if a_to_b {
        curr_fee_growth_global_input - yevepool.fee_growth_global_a
    } else {
        curr_fee_growth_global_input - yevepool.fee_growth_global_b
    };
...
mod swap_liquidity_tests {
    use super::*;
    use crate::util::{create_yevepool_reward_infos, test_utils::swap_test_fixture::*};
    ...
    fn zero_l_across_tick_range_b_to_a() {
        let swap_test_info = SwapTestFixture::new(SwapTestFixtureInfo {
            ...
            reward_infos: create_yevepool_reward_infos(100, 10),
