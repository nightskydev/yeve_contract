...
pub struct CollectReward<'info> {
    ...
    #[account(mut,
        constraint = reward_owner_account.mint == yevepool.reward_infos[reward_index as usize].mint
    )]
    pub reward_owner_account: Box<Account<'info, TokenAccount>>,

    #[account(mut, address = yevepool.reward_infos[reward_index as usize].vault)]
...
///
/// If the Yevepool reward vault does not have enough tokens, the maximum number of available
