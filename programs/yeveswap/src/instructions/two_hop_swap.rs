...
use crate::{
    ...
    state::{TickArray, Yevepool},
    util::{to_timestamp_u64, update_and_swap_yevepool, SwapTickSequence},
};
...
pub struct TwoHopSwap<'info> {
    ...
    #[account(mut)]
    pub yevepool_one: Box<Account<'info, Yevepool>>,

    #[account(mut)]
    pub yevepool_two: Box<Account<'info, Yevepool>>,

    #[account(mut, constraint = token_owner_account_one_a.mint == yevepool_one.token_mint_a)]
    pub token_owner_account_one_a: Box<Account<'info, TokenAccount>>,
    #[account(mut, address = yevepool_one.token_vault_a)]
    pub token_vault_one_a: Box<Account<'info, TokenAccount>>,

    #[account(mut, constraint = token_owner_account_one_b.mint == yevepool_one.token_mint_b)]
    pub token_owner_account_one_b: Box<Account<'info, TokenAccount>>,
    #[account(mut, address = yevepool_one.token_vault_b)]
    pub token_vault_one_b: Box<Account<'info, TokenAccount>>,

    #[account(mut, constraint = token_owner_account_two_a.mint == yevepool_two.token_mint_a)]
    pub token_owner_account_two_a: Box<Account<'info, TokenAccount>>,
    #[account(mut, address = yevepool_two.token_vault_a)]
    pub token_vault_two_a: Box<Account<'info, TokenAccount>>,

    #[account(mut, constraint = token_owner_account_two_b.mint == yevepool_two.token_mint_b)]
    pub token_owner_account_two_b: Box<Account<'info, TokenAccount>>,
    #[account(mut, address = yevepool_two.token_vault_b)]
    pub token_vault_two_b: Box<Account<'info, TokenAccount>>,

    #[account(mut, constraint = tick_array_one_0.load()?.yevepool == yevepool_one.key())]
    pub tick_array_one_0: AccountLoader<'info, TickArray>,

    #[account(mut, constraint = tick_array_one_1.load()?.yevepool == yevepool_one.key())]
    pub tick_array_one_1: AccountLoader<'info, TickArray>,

    #[account(mut, constraint = tick_array_one_2.load()?.yevepool == yevepool_one.key())]
    pub tick_array_one_2: AccountLoader<'info, TickArray>,

    #[account(mut, constraint = tick_array_two_0.load()?.yevepool == yevepool_two.key())]
    pub tick_array_two_0: AccountLoader<'info, TickArray>,

    #[account(mut, constraint = tick_array_two_1.load()?.yevepool == yevepool_two.key())]
    pub tick_array_two_1: AccountLoader<'info, TickArray>,

    #[account(mut, constraint = tick_array_two_2.load()?.yevepool == yevepool_two.key())]
    pub tick_array_two_2: AccountLoader<'info, TickArray>,

    #[account(seeds = [b"oracle", yevepool_one.key().as_ref()],bump)]
    /// CHECK: Oracle is currently unused and will be enabled on subsequent updates
    pub oracle_one: UncheckedAccount<'info>,

    #[account(seeds = [b"oracle", yevepool_two.key().as_ref()],bump)]
    /// CHECK: Oracle is currently unused and will be enabled on subsequent updates
...
pub fn handler(
    ...
    let yevepool_one = &mut ctx.accounts.yevepool_one;
    let yevepool_two = &mut ctx.accounts.yevepool_two;

    // Don't allow swaps on the same yevepool
    if yevepool_one.key() == yevepool_two.key() {
        return Err(ErrorCode::DuplicateTwoHopPool.into());
    }

    let swap_one_output_mint = if a_to_b_one {
        yevepool_one.token_mint_b
    } else {
        yevepool_one.token_mint_a
    };

    let swap_two_input_mint = if a_to_b_two {
        yevepool_two.token_mint_a
    } else {
        yevepool_two.token_mint_b
    };
