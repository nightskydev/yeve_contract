...
pub struct ModifyLiquidity<'info> {
    ...
    #[account(mut, constraint = token_owner_account_a.mint == yevepool.token_mint_a)]
    pub token_owner_account_a: Box<Account<'info, TokenAccount>>,
    #[account(mut, constraint = token_owner_account_b.mint == yevepool.token_mint_b)]
    pub token_owner_account_b: Box<Account<'info, TokenAccount>>,

    #[account(mut, constraint = token_vault_a.key() == yevepool.token_vault_a)]
    pub token_vault_a: Box<Account<'info, TokenAccount>>,
    #[account(mut, constraint = token_vault_b.key() == yevepool.token_vault_b)]
...
pub fn handler(
    ...
    let (delta_a, delta_b) = calculate_liquidity_token_deltas(
        ctx.accounts.yevepool.tick_current_index,
        ctx.accounts.yevepool.sqrt_price,
        &ctx.accounts.position,
        liquidity_delta,
    )?;
