...
declare_id!("2QE2SSJJvLdFAUkCzKkmX3UmiKsFbJ9qn8mmufeokx6p");
...
pub mod yeveswap {
    ...
    /// Withdraw liquidity from a position in the Yevepool. This call also updates the position's accrued fees and rewards.
    ///
    /// ### Authority
    /// - `position_authority` - authority that owns the token corresponding to this desired position.
    ///
    ...
    /// Collect the protocol fees accrued in this Yevepool
    ///
    /// ### Authority
    /// - `collect_protocol_fees_authority` - assigned authority in the YevepoolConfig that can collect protocol fees
    pub fn collect_protocol_fees(ctx: Context<CollectProtocolFees>) -> Result<()> {
        return instructions::collect_protocol_fees::handler(ctx);
    }

    /// Perform a swap in this Yevepool
    ///
    /// ### Authority
    ...
    /// - `SqrtPriceOutOfBounds` - User provided parameter `sqrt_price_limit` is over Yeveppool's max/min bounds for sqrt-price.
    ...
    /// Close a position in a Yevepool. Burns the position token in the owner's wallet.
    ///
    /// ### Authority
    ...
    /// Sets the default protocol fee rate for a YevepoolConfig
    /// Protocol fee rate is represented as a basis point.
    /// Only the current fee authority has permission to invoke this instruction.
    ///
    /// ### Authority
    /// - "fee_authority" - Set authority that can modify pool fees in the YevepoolConfig
    ///
    /// ### Parameters
    /// - `default_protocol_fee_rate` - Rate that is referenced during the initialization of a Yevepool using this config.
    ///
    /// #### Special Errors
    ...
    /// Sets the fee rate for a Yevepool.
    ...
    /// Sets the protocol fee rate for a Yevepool.
    /// Protocol fee rate is represented as a basis point.
    /// Only the current fee authority has permission to invoke this instruction.
    ///
    /// ### Authority
    ...
    /// Sets the fee authority for a YevepoolConfig.
    ...
    /// Sets the fee authority to collect protocol fees for a YevepoolConfig.
    /// Only the current collect protocol fee authority has permission to invoke this instruction.
    ///
    /// ### Authority
    /// - "fee_authority" - Set authority that can collect protocol fees in the YevepoolConfig
    ...
    /// Set the yevepool reward super authority for a YevepoolConfig
    /// Only the current reward super authority has permission to invoke this instruction.
    /// This instruction will not change the authority on any `YevepoolRewardInfo` yevepool rewards.
    ///
    /// ### Authority
    ...
    /// Perform a two-hop swap in this Yevepool
    ///
    /// ### Authority
    /// - "token_authority" - The authority to withdraw tokens from the input token account.
    ///
    ...
    /// - `DuplicateTwoHopPool` - Error if yevepool one & two are the same pool.
    pub fn two_hop_swap(
    ...
    /// Open a bundled position in a Yevepool. No new tokens are issued
    ...
    /// Close a bundled position in a Yevepool.
    ///
    /// ### Authority
    /// - `position_bundle_authority` - authority that owns the token corresponding to this desired position bundle.
    ///
