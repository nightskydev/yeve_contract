...
use super::YevepoolsConfig;

#[account]
#[derive(Default)]
pub struct Yevepool {
    pub yevepools_config: Pubkey, // 32
    pub yevepool_bump: [u8; 1],   // 1
    ...
    pub reward_infos: [YevepoolRewardInfo; NUM_REWARDS], // 384
}

// Number of rewards supported by Yevepools
...
impl Yevepool {
    ...
    pub fn initialize(
        ...
        self.yevepool_bump = [bump];

        self.tick_spacing = tick_spacing;
        self.tick_spacing_seed = self.tick_spacing.to_le_bytes();

        self.update_fee_rate(default_fee_rate)?;
        self.update_protocol_fee_rate(yevepools_config.default_protocol_fee_rate)?;
        ...
        self.reward_infos =
            [YevepoolRewardInfo::new(yevepools_config.reward_emissions_super_authority);
                NUM_REWARDS];

        Ok(())
    }

    /// Update all reward values for the Yevepool.
    ///
    /// # Parameters
    /// - `reward_infos` - An array of all updated yevepool rewards
    ...
    /// Update the reward authority at the specified Yevepool reward index.
...
/// Stores the state relevant for tracking liquidity mining rewards at the `Yevepool` level.
/// These values are used in conjunction with `PositionRewardInfo`, `Tick.reward_growths_outside`,
/// and `Yevepool.reward_last_updated_timestamp` to determine how many rewards are earned by open
/// positions.
#[derive(Copy, Clone, AnchorSerialize, AnchorDeserialize, Default, Debug, PartialEq)]
pub struct YevepoolRewardInfo {
    /// Reward token mint.
    pub mint: Pubkey,
...
impl YevepoolRewardInfo {
    /// Creates a new `YevepoolRewardInfo` with the authority set
...
pub struct YevepoolBumps {
    pub yevepool_bump: u8,
}

#[test]
fn test_yevepool_reward_info_not_initialized() {
    let reward_info = YevepoolRewardInfo::default();
    assert_eq!(reward_info.initialized(), false);
}

#[test]
fn test_yevepool_reward_info_initialized() {
    let reward_info = &mut YevepoolRewardInfo::default();
    reward_info.mint = Pubkey::new_unique();
    assert_eq!(reward_info.initialized(), true);
}

#[cfg(test)]
pub mod yevepool_builder {
    use super::{Yevepool, YevepoolRewardInfo, NUM_REWARDS};

    #[derive(Default)]
    pub struct YevepoolBuilder {
        liquidity: u128,
        tick_spacing: u16,
    ...
    impl YevepoolBuilder {
        pub fn new() -> Self {
            Self {
                reward_infos: [YevepoolRewardInfo::default(); NUM_REWARDS],
                ..Default::default()
            }
        }
        ...
        pub fn reward_info(mut self, index: usize, reward_info: YevepoolRewardInfo) -> Self {
            self.reward_infos[index] = reward_info;
            self
        }

        pub fn reward_infos(mut self, reward_infos: [YevepoolRewardInfo; NUM_REWARDS]) -> Self {
            self.reward_infos = reward_infos;
            self
        }
