...
impl Tick {
    ...
    /// Check that the tick index is a valid start tick for a tick array in this yevepool
    /// A valid start-tick-index is a multiple of tick_spacing & number of ticks in a tick-array.
    ///
    /// # Parameters
    /// - `tick_index` - A i32 integer representing the tick index
    /// - `tick_spacing` - A u8 integer of the tick spacing for this yevepool
    ///
    /// # Returns
    /// - `true`: The tick index is a valid start-tick-index for this yevepool
    /// - `false`: The tick index is not a valid start-tick-index for this yevepool
...
impl TickArray {
    ...
    ///
    /// # Parameters
    /// - `yevepool` - the tick index the desired Tick object is stored in
    /// - `start_tick_index` - A u8 integer of the tick spacing for this yevepool
    ///
    /// # Errors
    /// - `InvalidStartTick`: - The provided start-tick-index is not an initializable tick index in this yevepool w/ this tick-spacing.
    pub fn initialize(
        &mut self,
        yevepool: &Account<Yevepool>,
        start_tick_index: i32,
    ) -> Result<()> {
        if !Tick::check_is_valid_start_tick(start_tick_index, yevepool.tick_spacing) {
    ...
    /// - `TickNotFound`: - The provided tick-index is not an initializable tick index in this yevepool w/ this tick-spacing.
